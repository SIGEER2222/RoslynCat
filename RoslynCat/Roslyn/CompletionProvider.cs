...
namespace RoslynCat.Roslyn
{
    public class CompletionProvider
    {
        ...
        public async Task<string> GetCodeAsync(string code , CodeType codeType) => codeType switch
        {
            CodeType.QuickFix =>await CodeFix(code,_workSpace.Document),
        };
        private async Task<string> CodeFix(string code,Document document)
        {
            ...
            foreach (Diagnostic diagnostic in compilation.GetDiagnostics())
            {
                ...
                {
                    string missingName = diagnostic.GetMessage().Split(@"“")[1];
                    string identifier = FindIdentifier(compilation, missingName);
                    code = code.Replace(missingName, identifier);
                }
            }
            return code;
        }

        static string FindIdentifier(Compilation compilation, string identifierName)
        {
            INamespaceSymbol[] namespaces = compilation.GlobalNamespace.GetNamespaceMembers().ToArray();

            foreach (INamespaceSymbol ns in namespaces)
            {
                string identifier = FindIdentifierInNamespace(ns, identifierName);
                if (identifier != null)
                {
                    return identifier;
                }
        ...
        static string FindIdentifierInNamespace(INamespaceSymbol ns, string identifierName)
        {
            ISymbol identifier = ns.GetMembers(identifierName).FirstOrDefault();
            if (identifier != null)
            {
            ...
            {
                string identifier2 = FindIdentifierInNamespace(nestedNs, identifierName);
                if (identifier2 != null)
                {
                    return identifier2;
                }
